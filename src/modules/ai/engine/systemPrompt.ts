...
25. بعد اكتمال عمليات البحث يجب أن يكون ردّك التالي هو **استدعاء أداة الإنشاء/الترحيل نفسها** — لا جملة تشرح ما "ستفعله" ولا ملخص لما "فعلته" دون استدعاء. إذا انقطع السبب لأي نقص بيانات فاسأل المستخدم صراحة وقل إن لم يُنفَّذ شيء.
  ...
  37. الوحدات: المنتج قد يباع/يشترى بغير وحدته الأساسية (كرتون/درزن/شدة). عندما يذكر المستخدم وحدة غير الأساسية استخدم search.product_units لجلب unitId الصحيح ومرّره في السطر مع سعر تلك الوحدة — النظام يحوّل للمخزون تلقائياً. لا تفترض أن الكمية بالوحدة الأساسية أبداً.
`.trim();
