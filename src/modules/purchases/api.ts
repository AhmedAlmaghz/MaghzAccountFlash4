...
export const purchasesApi = {
  ...
  async postInvoice(id: string, companyId: string, _userId?: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const adapter = await getDbAdapter();
      const check = await adapter.query(
        'SELECT supplier_id, total_amount, paid_amount, subtotal, discount_amount, vat_amount, invoice_number, date, payment_type, cash_box_id, base_currency_amount, exchange_rate FROM purchase_invoices WHERE id = $1::uuid AND company_id = $2::uuid AND status = $3',
        [id, companyId, 'draft']
      );
      if (!check.success || !check.rows?.[0]) {
      ...
      // Resolve discount account if needed
      ...
      // Resolve posting accounts (inventory/creditors/VAT + optional variance)
