...
export const PaymentVouchersPage: React.FC = () => {
  ...
  const voucherFilters = useMemo(
    () => ({ status: statusFilter || undefined, search: search || undefined, paymentMethod: methodFilter || undefined }),
  ...
  const { vouchers, total, page, pageSize, isLoading, goToPage, changePageSize, create, update, remove } =
    usePaymentVouchersPaginated(activeCompany?.id || '', voucherFilters);
  ...
  const [form, setForm] = useState<Partial<PaymentVoucher> & { amountApplied?: number; baseCurrencyApplied?: number; invoiceId?: string }>({
    paymentMethod: 'cash',
    status: 'draft',
  ...
  const handlePrint = useCallback(
    (voucher: PaymentVoucher) => {
      printDocument({
        ...
        lines: [
          {
            description: t('accounting.paymentVouchers'),
            productName: voucher.supplierName,
            quantity: 1,
  ...
  const handlePost = async (voucher: PaymentVoucher) => {
    ...
    // Unified atomic posting via the API (JE + supplier balance + status flip).
    const result = await accountingApi.postVoucher(voucher.id, activeCompany.id, 'payment', user?.id || '');
    setPostingId(null);
    if (result.success) {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      amountApplied: form.invoiceId ? Number(form.amountApplied) || 0 : 0,
      currencyCode: form.currencyCode || YER_CODE,
      exchangeRate: form.exchangeRate ?? 1,
      baseCurrencyAmount: (Number(form.amount) || 0) * (form.exchangeRate ?? 1),
      baseCurrencyApplied: form.invoiceId ? (Number(form.amountApplied) || 0) * (form.exchangeRate ?? 1) : 0,
  ...
  const handleExportExcel = useCallback(() => {
    const cols = [
      ...
      { key: 'supplierName', header: t('accounting.supplier'), width: 24 },
    ...
    exportToExcel(
      ...
      cols,
      `payment_vouchers_${new Date().toISOString().split('T')[0]}`,
    );
  }, [vouchers, t]);
  ...
  const handleExportPdf = useCallback(() => {
    const cols = [
      ...
      { key: 'supplierName', header: t('accounting.supplier') },
    ...
    exportToPDF(
      ...
      cols,
      `payment_vouchers_${new Date().toISOString().split('T')[0]}`,
      { title: t('accounting.paymentVouchers'), rtl: true, companyName: activeCompany?.name },
    );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        width: '120px',
        render: (row: PaymentVoucher) => <span className="tabular-nums text-xs text-slate-700 dark:text-slate-300">{row.date ? formatDate(row.date) : '-'}</span>,
      },
      {
        key: 'supplierName',
        header: t('accounting.supplier'),
        render: (row: PaymentVoucher) => (
      ...
      },
      {
        ...
        width: '110px',
        render: (row: PaymentVoucher) => <StatusBadge status={row.status} size="sm" />,
      },
      {
        key: 'createdBy',
        header: t('accounting.createdBy'),
        width: '110px',
        render: (row: PaymentVoucher) => <span className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[100px] inline-block">{getUserName(row.createdBy)}</span>,
      },
      {
        key: 'actions',
        header: t('edit'),
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-rose-500 to-rose-600 dark:from-rose-600 dark:to-rose-700 flex items-center justify-center shadow-sm">
              <Banknote size={22} className="text-white" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50 tracking-tight">{t('accounting.paymentVouchers')}</h1>
              <p className="text-sm text-slate-500 dark:text-slate-400">سندات الصرف — مدفوعات الموردين والمصروفات</p>
            </div>
          </div>
          <Can action="create" module="accounting">
            <Button leftIcon={<Plus size={16} />} onClick={() => { resetForm(); setIsOpen(true); }} className="shadow-sm self-start sm:self-auto">
              {t('accounting.newPaymentVoucher')}
            </Button>
          </Can>
        </div>
          ...
          <div className="flex flex-col xl:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder={`${t('search')} — ${t('accounting.voucherNumber')} / ${t('accounting.supplier')}`}
        ...
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1.5 flex items-center gap-1">
                  <Building2 size={12} /> {t('accounting.supplier')}
                </label>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1.5">{t('accounting.expenseAccount')}</label>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <Input label={t('sales.exchangeRate')} type="number" min={0} step="0.0001" value={String(form.exchangeRate ?? 1)} onChange={(e) => setForm({ ...form, exchangeRate: Number(e.target.value) || 1 })} />
            </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (confirmDelete) {
            ...
            if (result?.success) addToast('success', t('accounting.paymentVoucher.deleted'));
