...
export const salesApi = {
  ...
  async postInvoice(id: string, companyId: string, _userId?: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const check = await adapter.query(
        'SELECT customer_id, total_amount, paid_amount, subtotal, vat_amount, invoice_number, date, payment_type, cash_box_id, base_currency_amount, exchange_rate FROM sales_invoices WHERE id = $1::uuid AND company_id = $2::uuid AND status = $3',
        [id, companyId, 'draft']
      );
      if (!check.success || !check.rows?.[0]) {
