...
describe('Migration 0020: companies audit columns', () => {
  const migrationSql = readFileSync(join(MIGRATIONS_DIR, '0020_companies_audit.sql'), 'utf-8');

  it('adds created_by and updated_by to companies', () => {
    expect(migrationSql).toMatch(/ALTER TABLE companies ADD COLUMN IF NOT EXISTS created_by uuid/);
    expect(migrationSql).toMatch(/ALTER TABLE companies ADD COLUMN IF NOT EXISTS updated_by uuid/);
  });
  ...
  it('journal registers 0020 and count mirrors sql files', () => {
    const journal = JSON.parse(readFileSync(join(MIGRATIONS_DIR, 'meta', '_journal.json'), 'utf-8'));
    expect(journal.entries.some((e: { tag: string }) => e.tag === '0020_companies_audit')).toBe(true);
    expect(journal.entries.length).toBe(readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith('.sql')).length);
  });

  it('pgliteAdapter registers 0020 in its hand-maintained MIGRATIONS list', () => {
    const pglite = readFileSync(join(process.cwd(), 'src/core/database/adapters/pgliteAdapter.ts'), 'utf-8');
    expect(pglite).toMatch(/0020_companies_audit\.sql\?raw/);
    expect(pglite).toMatch(/\{ name: '0020_companies_audit', sql: companiesAudit \}/);
  });

  it('Drizzle schema exposes createdBy/updatedBy on companies', () => {
    const schema = readFileSync(join(process.cwd(), 'src/core/database/schema/core.ts'), 'utf-8');
    expect(schema).toMatch(/createdBy: uuid\('created_by'\)/);
    expect(schema).toMatch(/updatedBy: uuid\('updated_by'\)/);
  });

  it('unified updateCompany writes updated_by (regression: companies save)', () => {
    const rpc = readFileSync(join(process.cwd(), 'electron/dbHandler.js'), 'utf-8');
    expect(rpc).toMatch(/UPDATE companies SET[\s\S]*?updated_by/);
    const pglite = readFileSync(join(process.cwd(), 'src/core/database/adapters/pgliteAdapter.ts'), 'utf-8');
...
describe('Migration 0029: users/roles hardening', () => {
  const migrationSql = readFileSync(join(MIGRATIONS_DIR, '0029_users_roles_hardening.sql'), 'utf-8');

  it('adds hardened constraints to users/roles', () => {
    expect(migrationSql).toMatch(/ALTER TABLE users/);
    expect(migrationSql).toMatch(/IF NOT EXISTS/);
  });

  it('journal registers 0029 and count mirrors sql files', () => {
    const journal = JSON.parse(readFileSync(join(MIGRATIONS_DIR, 'meta', '_journal.json'), 'utf-8'));
    expect(journal.entries.some((e: { tag: string }) => e.tag === '0029_users_roles_hardening')).toBe(true);
    expect(journal.entries.length).toBe(readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith('.sql')).length);
  });

  it('pgliteAdapter registers 0029', () => {
    const pglite = readFileSync(join(process.cwd(), 'src/core/database/adapters/pgliteAdapter.ts'), 'utf-8');
    expect(pglite).toMatch(/0029_users_roles_hardening\.sql\?raw/);
  });
});
...
describe('Migration 0032: perpetual inventory valuation (Phase 1 FIN-1)', () => {
  const migrationSql = readFileSync(join(MIGRATIONS_DIR, '0032_inventory_valuation.sql'), 'utf-8');
  ...
  it('journal registers 0029 and count mirrors sql files', () => {
    const journal = JSON.parse(readFileSync(join(MIGRATIONS_DIR, 'meta', '_journal.json'), 'utf-8'));
    expect(journal.entries.some((e: { tag: string }) => e.tag === '0032_inventory_valuation')).toBe(true);
    expect(journal.entries.length).toBe(readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith('.sql')).length);
  });

  it('pgliteAdapter registers 0029 in its hand-maintained MIGRATIONS list', () => {
    const pglite = readFileSync(join(process.cwd(), 'src/core/database/adapters/pgliteAdapter.ts'), 'utf-8');
    expect(pglite).toMatch(/0032_inventory_valuation\.sql\?raw/);
    expect(pglite).toMatch(/\{ name: '0032_inventory_valuation', sql: inventoryValuation \}/);
  });
...
describe('Migration 0033: FX revaluation (Phase 2 FIN-2)', () => {
  const migrationSql = readFileSync(join(MIGRATIONS_DIR, '0033_fx_revaluation.sql'), 'utf-8');
  ...
  it('journal registers 0030 and count mirrors sql files', () => {
    const journal = JSON.parse(readFileSync(join(MIGRATIONS_DIR, 'meta', '_journal.json'), 'utf-8'));
    expect(journal.entries.some((e: { tag: string }) => e.tag === '0030_pos_receipt_and_line_cost')).toBe(true);
    expect(journal.entries.some((e: { tag: string }) => e.tag === '0033_fx_revaluation')).toBe(true);
    expect(journal.entries.length).toBe(readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith('.sql')).length);
  });
...
describe('Migration 0031: Dedicated discount accounts', () => {
  ...
  it('pgliteAdapter registers 0031 in its hand-maintained MIGRATIONS list', () => {
    ...
    expect(pglite).toMatch(/0031_discount_accounts\.sql\?raw/);
    expect(pglite).toMatch(/\{ name: '0031_discount_accounts', sql: discountAccounts \}/);
    expect(pglite).toMatch(/0033_fx_revaluation\.sql\?raw/);
    expect(pglite).toMatch(/\{ name: '0033_fx_revaluation', sql: fxRevaluation \}/);
  });
...
describe('Migration 0035: year-end close + fixed assets + accounting periods (Phase 5)', () => {
  const migrationSql = readFileSync(join(MIGRATIONS_DIR, '0035_phase5_close_assets_periods.sql'), 'utf-8');
  ...
  it('journal registers 0032 and count mirrors sql files', () => {
    const journal = JSON.parse(readFileSync(join(MIGRATIONS_DIR, 'meta', '_journal.json'), 'utf-8'));
    expect(journal.entries.some((e: { tag: string }) => e.tag === '0035_phase5_close_assets_periods')).toBe(true);
    expect(journal.entries.length).toBe(readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith('.sql')).length);
  });

  it('pgliteAdapter registers 0032 in its hand-maintained MIGRATIONS list', () => {
    const pglite = readFileSync(join(process.cwd(), 'src/core/database/adapters/pgliteAdapter.ts'), 'utf-8');
    expect(pglite).toMatch(/0035_phase5_close_assets_periods\.sql\?raw/);
    expect(pglite).toMatch(/\{ name: '0035_phase5_close_assets_periods', sql: phase5CloseAssetsPeriods \}/);
  });
...
describe('Migration 0036: leave-provision account (Phase 5 IAS 19)', () => {
  const migrationSql = readFileSync(join(MIGRATIONS_DIR, '0036_leave_provision_account.sql'), 'utf-8');
  ...
  it('journal registers 0033 and count mirrors sql files', () => {
    const journal = JSON.parse(readFileSync(join(MIGRATIONS_DIR, 'meta', '_journal.json'), 'utf-8'));
    expect(journal.entries.some((e: { tag: string }) => e.tag === '0036_leave_provision_account')).toBe(true);
    expect(journal.entries.length).toBe(readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith('.sql')).length);
  });

  it('pgliteAdapter registers 0033 in its hand-maintained MIGRATIONS list', () => {
    const pglite = readFileSync(join(process.cwd(), 'src/core/database/adapters/pgliteAdapter.ts'), 'utf-8');
    expect(pglite).toMatch(/0036_leave_provision_account\.sql\?raw/);
    expect(pglite).toMatch(/\{ name: '0036_leave_provision_account', sql: leaveProvisionAccount \}/);
  });
...
describe('Migration 0037: treasury box accounts (Phase 6)', () => {
  const migrationSql = readFileSync(join(MIGRATIONS_DIR, '0037_treasury_box_accounts.sql'), 'utf-8');
  ...
  it('journal registers 0034 and count mirrors sql files', () => {
    const journal = JSON.parse(readFileSync(join(MIGRATIONS_DIR, 'meta', '_journal.json'), 'utf-8'));
    expect(journal.entries.some((e: { tag: string }) => e.tag === '0037_treasury_box_accounts')).toBe(true);
    expect(journal.entries.length).toBe(readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith('.sql')).length);
  });

  it('pgliteAdapter registers 0034 in its hand-maintained MIGRATIONS list', () => {
    const pglite = readFileSync(join(process.cwd(), 'src/core/database/adapters/pgliteAdapter.ts'), 'utf-8');
    expect(pglite).toMatch(/0037_treasury_box_accounts\.sql\?raw/);
    expect(pglite).toMatch(/\{ name: '0037_treasury_box_accounts', sql: treasuryBoxAccounts \}/);
  });
